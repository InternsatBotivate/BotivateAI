...
function onOpen() {
  SpreadsheetApp.getUi()
      ...
      .addItem('▶️ Run Full Sync For All Sheets', 'runDatabaseSync')
      .addToUi();
}
...
function runDatabaseSync() {
  ...
  if (tableNames.length === 0) {
    console.log("ℹ️ No sheet data found to sync.");
    return;
  }
...
function insertSupabaseRows(tableName, rows, supabaseUrl, serviceKey) {
  ...
  try {
    ...
    if (responseCode === 201) {
      ...
      console.error(`❌ Insert failed for "${tableName}". Code: ${responseCode}. Error: ${responseBody}`);
      return false;
    }
  } catch (e) {
    console.error(`❌ Exception during insert for "${tableName}": ${e}`);
    return false;
  }
}
...
function callSupabaseRpc(functionName, params, supabaseUrl, serviceKey) {
  ...
  try {
    ...
    if (responseCode === 200) {
      console.log(`✅ RPC "${functionName}" successful. Response: ${responseBody}`);
      return true;
    } else {
      console.error(`❌ RPC "${functionName}" failed. Code: ${responseCode}. Error: ${responseBody}`);
      return false;
    }
  } catch (e) {
    console.error(`❌ Exception calling RPC "${functionName}": ${e}`);
    return false;
  }
}